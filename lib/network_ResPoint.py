...
class FiLMFusion(nn.Module):
    def __init__(self, C_img=256, C_pc=256):
        super().__init__()
        self.gamma = nn.Sequential(nn.AdaptiveAvgPool1d(1),
                                   nn.Conv1d(C_img, C_pc, 1),
                                   nn.ReLU())
        self.beta  = nn.Sequential(nn.AdaptiveAvgPool1d(1),
                                   nn.Conv1d(C_img, C_pc, 1),
                                   nn.ReLU())
    def forward(self, pc_feat, img_feat):
        # img_feat: (B, C_img, N) 或 (B,C_img) 先 pool
        img_pool = F.adaptive_avg_pool1d(img_feat, 1)  # (B, C_img, 1)
        gamma = self.gamma(img_pool)  # (B, C_pc, 1)
        beta  = self.beta(img_pool)
        out = pc_feat * (1 + gamma) + beta
        return out

class FiLM(nn.Module):
    def __init__(self, cond_dim, feature_dim):
        super(FiLM, self).__init__()
        self.gamma = nn.Conv1d(cond_dim, feature_dim, 1)
        self.beta = nn.Conv1d(cond_dim, feature_dim, 1)
...
class PoseNetFeat(nn.Module):
    def __init__(self, num_points):
        ...
        self.film1 = FiLM(128, 128)  # emb 调制 x_128
        self.film2 = FiLM(256, 256)  # emb 调制 x
