...
intrinsic_matrix = {
    'linemod': np.array([[1760.686179, 0.,         752.063486],
                        [0.,        1767.366207,  601.038088],
                        [0.,        0.,         1.]]),
...
class PoseTransformer(object):
    ...
    def __init__(self, class_type):
        ...
        self.xyz_pattern = os.path.join(lm_occ_pth,'models(tihuan)/{}/{}.xyz')
...
class Basic_Utils():
    ...
    def __init__(self, config):
        self.xmap = np.array([[j for i in range(1440)] for j in range(1080)])
        self.ymap = np.array([[i for i in range(1440)] for j in range(1080)])
        self.config = config
    ...
    def dpt_2_cld(self, dpt, cam_scale, K):
        if len(dpt.shape) > 2:
            dpt = dpt[:, :, 0]
        msk_dp = dpt > 1e-6
        choose = msk_dp.flatten().nonzero()[0].astype(np.uint32) # 有效像素的索引
        if len(choose) < 1:
            return None, None
    ...
    def get_normal(self, cld):
        ...
        cloud.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamKNN(knn=20)  # 使用 KNN 搜索
            # 或者用半径搜索：
            # search_param=o3d.geometry.KDTreeSearchParamRadius(radius=0.1)
        )
        #👇归一化法向量（推荐）
        cloud.normalize_normals()
        #👇获取法向量数组
        n = np.asarray(cloud.normals)  # shape: (N, 3)
        return n

    def get_normal_map(self, nrm, choose):
        nrm_map = np.zeros((1080, 1440, 3), dtype=np.uint8)
        nrm = nrm[:, :3]
        nrm[np.isnan(nrm)] = 0.0
        nrm[np.isinf(nrm)] = 0.0
        print("nrm range:", nrm.min(), "~", nrm.max())  # 👈 调试
        nrm_color = ((np.clip(nrm, -1.0, 1.0) + 1.0) * 127.5).astype(np.uint8)
        print("nrm range:", nrm.min(), "~", nrm.max())  # 👈 调试
        nrm_map = nrm_map.reshape(-1, 3)
        nrm_map[choose, :] = nrm_color
        nrm_map = nrm_map.reshape((1080, 1440, 3))
        return nrm_map

    def get_rgb_pts_map(self, pts, choose):
        pts_map = np.zeros((1080, 1440, 3), dtype=np.uint8)
        pts = pts[:, :3]
        pts[np.isnan(pts)] = 0.0
        pts[np.isinf(pts)] = 0.0
        # 方法2：假设坐标在 [-1, 1] 范围内（常见于归一化物体坐标）
        # 归一化到 [-1, 1]
        pts_min = pts.min(axis=0)
        pts_max = pts.max(axis=0)

        eps = 1e-8
        pts_norm = 2 * (pts - pts_min) / (pts_max - pts_min + eps) - 1.0
        pts_norm = np.clip(pts_norm, -1.0, 1.0)
        pts_color = ((pts_norm + 1.0) * 127.5).astype(np.uint8)

        pts_map = pts_map.reshape(-1, 3)
        pts_map[choose, :] = pts_color
        pts_map = pts_map.reshape((1080, 1440, 3))
        return pts_map
    ...
    def get_ycb_ply_mdl(
        ...
        ply_pattern = os.path.join(
            self.config.ycb_root, '/models(tihuan)',
            '{}/textured.ply'
        )
    ...
    def get_pointxyz(
        ...
        if ds_type == "ycb":
            ...
            ptxyz_pth = os.path.join(
                '/media/q/SSD2T/1linux/Linemod3_dataset/Linemod_preprocessed/models',
                'obj_%06d.ply' % cls
            )
            pointxyz = self.ply_vtx(ptxyz_pth) / 1000.0
            dellist = [j for j in range(0, len(pointxyz))]
            dellist = random.sample(dellist, len(pointxyz) - 1300)
