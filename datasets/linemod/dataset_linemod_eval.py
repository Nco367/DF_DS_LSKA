...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine):
        ...
        # self.trancolor = transforms.ColorJitter(0.2, 0.2, 0.2, 0.02) #调用 transform(image) 时，图像的亮度、对比度、饱和度和色调都会被随机调整
    ...
    def __getitem__(self, index):
        ...
        img_masked = img
        # 如果 img_masked 是 numpy，就转成 tensor
        if isinstance(img_masked, np.ndarray):
            img_masked = torch.from_numpy(img_masked.astype(np.float32))
        else:
            # 已经是 tensor 了，就直接转 dtype
        ...
        return torch.from_numpy(cloud.astype(np.float32)), \
               ...
               self.norm(img_masked.to(torch.float32)), \
