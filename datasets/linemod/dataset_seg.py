...
class PoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(label.shape) == 3:  # 如果是RGB格式的标签，提取单通道
            label = label[:, :, 0]
        label = torch.from_numpy(label.astype(np.int64))  # 转换为LongTensor
        ...
        img = self.norm(torch.from_numpy(img.astype(np.float32)))  # 归一化+转为张量
        return img, label
