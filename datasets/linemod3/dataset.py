...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine):
        ...
        self.objlist_now = [2,3,10]  # 对象列表～________________________________________________________________________________________________________________________________________________________________
        ...
        for item in self.objlist_now:
            ...
            # 加载模型的ground truth文件
            meta_file = open('{0}/data/{1}/gt.yaml'.format(self.root, '%02d' % item), 'r')
        ...
        # 创建地图
        self.xmap, self.ymap = np.meshgrid(np.arange(1440), np.arange(1080))

        self.num = num #
        self.add_noise = add_noise #
        #self.trancolor = transforms.ColorJitter(0.2, 0.2, 0.2, 0) #调用 transform(image) 时，图像的亮度、对比度、饱和度和色调都会被随机调整

        self.trancolor= A.Compose([
                            A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=1.0),   # 亮度、对比度范围
                            A.HueSaturationValue(
                                hue_shift_limit=18,    # 色调 [-18, 18] 度 ≈ hue=0.05
                                sat_shift_limit=0.2*255,  # 饱和度 [0.8,1.2]，像素值范围用 0–255，≈ ±51
                                val_shift_limit=0,     # 明度（相当于亮度，已在前面调过，这里设 0）
                                p=1.0
                            )
                        ])
        ...
        self.num_pt_mesh_small = 700
        self.symmetry_obj_idx = [2,3,10]
    ...
    def __getitem__(self, index):
        ...
        # ——————————————————————————————————————————————————img_masked——————————————————————————————————————————————————————
        if self.add_noise:
            augmented = self.trancolor(image=ori_img)
            img = augmented["image"]
        img = np.array(img)[:, :, :3]
...
border_list = [-1, 40, 80, 120, 160, 200, 240, 280, 320, 360, 400, 440, 480, 520, 560, 600, 640, 680, 720, 760, 800, 840, 880, 920, 960, 1000, 1040, 1080, 1120, 1160, 1200, 1240, 1280, 1320, 1360, 1400, 1440,1480]
img_width = 1080
img_length = 1440
