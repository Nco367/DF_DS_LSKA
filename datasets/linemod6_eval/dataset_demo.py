...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine):
        ...
        self.objlist_now = [3]  # 对象列表～________________________________________________________________________________________________________________________________________________________________
        ...
        for item in self.objlist_now:
            if self.mode == 'train':
                ...
                input_file = open('{0}/data/{1}/test_demo.txt'.format(self.root, '%02d' % item))
            while 1:
                # 物体ID 累加
                item_count += 1
                # 加载训练帧、测试帧编号列表
    ...
    def __getitem__(self, index):
        ...
        p_img_masked = np.transpose(img_masked, (1, 2, 0))
        p_img = np.transpose(img, (1,2,0))
        plt.imsave('evaluation_result/{0}_ori.png'.format(index), p_img)
        plt.imsave('evaluation_result/{0}_input.png'.format(index), p_img_masked)
        ...
        # ——————————————————————————————————————————————————choose 掩码所选的点？——————————————————————————————————————————————————————
        choose = mask[rmin:rmax, cmin:cmax].flatten().nonzero()[0] # 从图像掩码中提取有效点索引
        if len(choose) == 0:
            cc = torch.LongTensor([0]) # LongTensor 经常用于存储整数类型的标签或索引
            return(cc, cc, cc, cc, cc, cc)

        if len(choose) > self.num:
            c_mask = np.zeros(len(choose), dtype=int) # 创建全零掩码
            c_mask[:self.num] = 1 # 将前 self.num 个位置标记为 1
            np.random.shuffle(c_mask) # 打乱掩码顺序，实现随机选择
            choose = choose[c_mask.nonzero()] # 通过非零掩码索引，随机选择 self.num 个点
        else: # 如果有效点不足 self.num，进行填充；补充到设定值
            choose = np.pad(choose, (0, self.num - len(choose)), 'wrap') # 循环填充（例如 [1,2] 填充到长度 5 → [1,2,1,2,1]）
        
        depth_masked = depth[rmin:rmax, cmin:cmax].flatten()[choose][:, np.newaxis].astype(np.float32) # 根据mask删减，提取深度图中对应索引的深度值
        # [:, np.newaxis]：增加维度（形状从 (N,) 变为 (N, 1)）;.astype(np.float32)：转换为单精度浮点数
        xmap_masked = self.xmap[rmin:rmax, cmin:cmax].flatten()[choose][:, np.newaxis].astype(np.float32)
        ymap_masked = self.ymap[rmin:rmax, cmin:cmax].flatten()[choose][:, np.newaxis].astype(np.float32)  # 提取 x/y 坐标图中对应索引的坐标值

        choose = np.array([choose])

        # ——————————————————————————————————————————————————cloud 掩码处根据深度图计算的点云（来源为深度图）——————————————————————————————————————————————————————
        cam_scale = 1.0
        ...
        # ——————————————————————————————————————————————————model_points 删减后的ply点云（来源为模型）——————————————————————————————————————————————————————
        model_points = self.pt[obj] * 1000.0 # 单位转化
        print("原始点云数量:", len(self.pt[obj]))  # 预期 > self.num_pt_mesh_small
        dellist = [j for j in range(0, len(model_points))] # 要删除的列表：生成一个包含所有点云索引的列表
        dellist = random.sample(dellist, len(model_points) - self.num_pt_mesh_small) # 随机删除：从所有点云索引中随机选择要删除的点的索引
        model_points = np.delete(model_points, dellist, axis=0) # 简化后的模型点云（来源为模型）：根据待删除索引列表，从原始点云中删除指定点
...
def ply_vtx(path):
    ...
    return np.vstack([vertex_data["x"], vertex_data["y"], vertex_data["z"]]).T.astype(np.float32)
#
# import struct
#
#
