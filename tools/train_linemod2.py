...
from datasets.linemod2.dataset import PoseDataset as PoseDataset_linemod2
...
# 数据集参数设置
    if opt.dataset == 'ycb':
        ...
        opt.num_objects = 3
        opt.num_points = 1000
        opt.outf = '/media/q/SSD2T/1linux/Linemod2_trained_mod' # 设置训练过程中保存模型的文件夹路径
        opt.log_dir = '/media/q/SSD2T/1linux/Linemod2_log' # 设置训练过程中日志保存的目录
...
# 数据加载
    if opt.dataset == 'ycb':
        ...
        dataset = PoseDataset_linemod2('train', opt.num_points, True, opt.dataset_root, opt.noise_trans, opt.refine_start)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=opt.workers)
    if opt.dataset == 'ycb':
        test_dataset = PoseDataset_ycb('test', opt.num_points, False, opt.dataset_root, 0.0, opt.refine_start)
    elif opt.dataset == 'linemod':
        test_dataset = PoseDataset_linemod2('test', opt.num_points, False, opt.dataset_root, 0.0, opt.refine_start)
