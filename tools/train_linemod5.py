...
# 数据集参数设置
    if opt.dataset == 'ycb':
        ...
        opt.num_objects = 1
        opt.num_points = 1000
        opt.outf = '/media/q/SSD2T/1linux/Linemod5_trained_mod' # 设置训练过程中保存模型的文件夹路径
        opt.log_dir = '/media/q/SSD2T/1linux/Linemod5_log' # 设置训练过程中日志保存的目录
        opt.repeat_epoch = 8 # 设置每个物体训练的重复轮数为 20
    else:
